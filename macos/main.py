...
def main() -> int:
    if sys.platform != "darwin":
        print("这是 macOS 专用工程，请拷到 Mac 上运行或打包。", file=sys.stderr)
        return 1
...
class ControlWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Mac 悬浮窗打包测试")
        ...
        refresh = QPushButton("重新打上 sharingType")
        refresh.clicked.connect(self._refresh_status)

        hint = QLabel(
            "这个控制窗口会出现在屏幕共享里。\n"
            "白色字幕悬浮窗在 14.2.1 上应被 sharingType=none 排除。\n"
            "测试：Cmd+Shift+4 截图，或 Zoom/腾讯会议共享整个屏幕。"
        )
